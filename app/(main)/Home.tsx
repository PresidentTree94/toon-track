...
export default function Home({ webtoonsData, completedData }: { webtoonsData: Toon[], completedData: Comp[] }) {
  ...
  const growthThreshold = webtoonsData.filter(item => item.data.length > 1);
  const subThreshold = webtoonsData.filter(item => item.data.length > 0);
  ...
  const hiatus = webtoonsData.filter(item => item.status === "Hiatus").length;
  const ongoing = webtoonsData.length - hiatus;

  const cards = [
    {heading: "Median Growth", number: growthThreshold.length > 0 ? medianGrowth.toFixed(0) + "%" : "N/E", subheading: `from ${growthThreshold.length}/${webtoonsData.length} Webtoons`},
    {heading: "Active Series", number: ongoing.toString(), subheading: `${hiatus} Hiatus, ${completedData.length} Completed`},
    {heading: "Median Subs", number: subThreshold.length > 0 ? condenseValue(medianSubs) : "N/E", subheading: `from ${subThreshold.length}/${webtoonsData.length} Webtoons`},
    {heading: "Sub Change", number: growthThreshold.length > 0 ? condenseValue(medianSubChange) : "N/E", subheading: `from ${growthThreshold.length}/${webtoonsData.length} Webtoons`}
  ];

  const graphs = [
    {title: "Median Growth Timeline", tagline: "How much a typical series is growing", data: calcMedianGrowthTimeline(webtoonsData)},
    {title: "Median Subs Timeline", tagline: "What a typical series following looks like", data: calcMedianSubsTimeline(webtoonsData)},
    {title: "Median Sub Change Timeline", tagline: "How much a typical series gained/lost", data: calcSubChangeTimeline(webtoonsData)}
  ];

  const [sortKey, setSortKey] = useState<"series" |"subs" | "growth">("subs");
  const sortedWebtoons = [...webtoonsData].sort((a, b) => {
  ...
  const DAY_LIMIT = 5;
  const KarlyWebtoons = webtoonsData.filter(w => w.owner === "Karly" && Math.floor((Date.now() - new Date(w.timestamp).getTime()) / MS_PER_DAY) <= DAY_LIMIT);
  const RachelleWebtoons = webtoonsData.filter(w => w.owner === "Rachelle" && Math.floor((Date.now() - new Date(w.timestamp).getTime()) / MS_PER_DAY) <= DAY_LIMIT);
  const changeOwnership = webtoonsData.filter(w => Math.floor((Date.now() - new Date(w.owner_time).getTime()) / MS_PER_DAY) <= DAY_LIMIT);
  const changeStatus = webtoonsData.filter(w => w.status_time && Math.floor((Date.now() - new Date(w.status_time).getTime()) / MS_PER_DAY) <= DAY_LIMIT);
  const completedRecently = completedData.filter(c => Math.floor((Date.now() - new Date(c.timestamp).getTime()) / MS_PER_DAY) <= DAY_LIMIT);
  const missingData = [...webtoonsData.filter(w => !w.genre || !w.thumbnail), ...completedData.filter(c => !c.genre || !c.thumbnail)];
